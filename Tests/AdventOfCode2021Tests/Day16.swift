@testable import AdventOfCode2021
import Foundation
import XCTest

final class Day16Tests: XCTestCase {
    func testLiteral() throws {
        var input = """
        110100101111111000101000
        """.map {  Int("\($0)")! }.dropFirst(0)
        let d = Day16()
        let packet = d.readPacket(input: &input)
        XCTAssert(packet?.intValue == 2021)
    }

    func testOperator1() throws {
        var input = """
        00111000000000000110111101000101001010010001001000000000
        """.map {  Int("\($0)")! }.dropFirst(0)
        let d = Day16()
        let packet = d.readPacket(input: &input)
        if case let .op(_, packets) = packet {
            XCTAssert(packets[0].intValue == 10)
            XCTAssert(packets[1].intValue == 20)
        }
    }
    func testOperator2() throws {
        var input = """
        11101110000000001101010000001100100000100011000001100000
        """.map {  Int("\($0)")! }.dropFirst(0)
        let d = Day16()
        let packet = d.readPacket(input: &input)
        if case let .op(_, packets) = packet {
            XCTAssert(packets[0].intValue == 1)
            XCTAssert(packets[1].intValue == 2)
            XCTAssert(packets[2].intValue == 3)
        }
    }

    func testReal() throws {
        let input = """
        
        """
        let d = Day16()
        let data = d.parse(input: input)
        XCTAssertEqual(d.partA(input: data), 904)
        XCTAssertEqual(d.partB(input: data), 200476472872)
        
    }
}
